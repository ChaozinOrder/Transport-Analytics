...
PLOT_FILE_NAME <- "chennaivelankannidea.png" # Name of the file to save the plot
...
df_grouped_inputs <- chennaivelankanniupdowncapacitycount

df_grouped_output <- aggregate(
  cbind(`DN COUNT`) ~ YEAR + CLASS,
  data = df,
  FUN = sum
)
...
selected_cols <- c("YEAR", "CLASS", "DN_PEAK_FREQUENCY", "DN_OFF_FREQUENCY", "DN COUNT")
...
output_vars_r <- c('DN COUNT') # R converts space to dot
